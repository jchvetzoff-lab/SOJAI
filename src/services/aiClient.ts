...
import { runMockChatCommand } from '@/features/ai/chat/mockEngine';
import type { ChatCommandInput, ChatCommandResult } from '@/features/ai/chat/types';
...
export interface AIClient {
  ...
  chatCommand(input: ChatCommandInput): Promise<ChatCommandResult>;
}
...
export const aiClient: AIClient = {
  ...
  async chatCommand(input) {
    if (flags.isDemo || flags.aiProvider === 'mock') {
      await delay(flags.demoLatencyMs);
      return runMockChatCommand(input);
    }
    ...
    if (!response.ok) {
      const payload: unknown = await response.json().catch(() => ({ error: 'Chat failed' }));
      const errorMessage = getApiErrorMessage(payload, response.status);
      throw new Error(errorMessage);
    }

    return (await response.json()) as ChatCommandResult;
  },
};
...
export async function chatCommand(input: ChatCommandInput): Promise<ChatCommandResult> {
  return aiClient.chatCommand(input);
}
